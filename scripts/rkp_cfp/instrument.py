...
class Objdump(object):
    ...
    def parse(self):
        ...
        with open(tmp, 'r') as f:
            for i, line in enumerate(f):
                virt_addr = None
                m = re.search(virt_addr_re, line.decode('utf-8'))
                if m:
                    virt_addr = _int(m.group('virt_addr'))

                self.lines.append((line, section_idx, virt_addr))

                m = re.searchre.search(r'Disassembly of section (?P<name>.*):', line.decode('utf-8'))
                if m:
                    section_idx = self.sections['section_idx'][m.group('name')]
                    continue

                m = re.search(common.fun_rec, line.decode('utf-8'))
                if m:
    ...
    def parse_insn(self, i):
        ...
        """
        line = self.line(i)
        m = re.search(Objdump.PARSE_INSN_RE, line.decode('utf-8'))
        insn = m.groupdict()
...
def parse_nm(vmlinux, symbols=None):
    ...
    last_name = None
    for line in f:
        m = re.search(NM_RE, line.decode('utf-8'))
        if m:
...
def parse_sections(vmlinux):
    ...
    while True:
        ...
        m = re.search(r'^Sections:', line.decode('utf-8'))
        if m:
            # first section
            next(it)
            continue
        ...
        m = re.search((
            ...
            r'\s+(?P<offset>{hex_re})'
            r'\s+(?P<align>[^\s]+)'
            ).format(hex_re=hex_re), line.decode('utf-8'))
        if m:
            section = {}
